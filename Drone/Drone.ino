...
// arrays for setting freq range, 
int sgnl[2] = {false, true}; // Signal on/off
int wave[2] = {SINE_WAVE, TRIANGLE_WAVE}; // Fundamental frquency waveform not using the square waves  SQUARE_WAVE, HALF_SQUARE_WAVE
...
int hmon[7] = {2, 3, 4, 5, 6, 7, 8}; // harmonic content hz multiplier on harmonics
...
// Variables for inputs and array indexes
...
const int pot2 = A5;          // select the input pin for the potentiometer
...
//  gen2.Reset(); 
  ...
  // button code, uncomment if not using that pin to avoid floating inputs
...
// define local variables
  ...
  trimindex = constrain(trimindex, 0, 66);  
...
// delay a bit and increment counter
  delay(20);
  ++count; 
} // loop
